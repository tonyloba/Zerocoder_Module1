...
def delete_task():
    ...
    if task:
        task_entry.delete(0, tk.END)
    # else:
    #     messagebox.showwarning("Delete", "Please enter a task.")
    deleted = False
    for listbox in [new_tasks_listbox, in_progress_listbox, closed_tasks_listbox]:
        for i in range(listbox.size()):
            if listbox.itemcget(i, "bg") == "lightblue":
                ...
                deleted = True
    if deleted:
        messagebox.showinfo("Delete", "Selected tasks were deleted.")
    elif not new_tasks_listbox.size() and not in_progress_listbox.size() and not closed_tasks_listbox.size():
        messagebox.showwarning("Delete", "No tasks to delete.")
    else:
        messagebox.showwarning("Delete", "No selected tasks to delete.")
    ...
    for listbox in [new_tasks_listbox, in_progress_listbox, closed_tasks_listbox]:
        for i in range(listbox.size()):
            if listbox.itemcget(i, "bg") == "lightblue":
                ...
                closed = True
    if closed:
        messagebox.showinfo("Close", "Selected tasks were closed.")
    elif not new_tasks_listbox.size() and not in_progress_listbox.size() and not closed_tasks_listbox.size():
        messagebox.showwarning("Close", "No tasks to close.")
    else:
        messagebox.showwarning("Close", "No selected tasks to close.")
    ...
    for listbox in [new_tasks_listbox, in_progress_listbox, closed_tasks_listbox]:
        for i in range(listbox.size()):
            if listbox.itemcget(i, "bg") == "lightblue":
                ...
                inWork=True
    if inWork:
        messagebox.showinfo("Move to Work", "Selected tasks were moved to work.")
    elif not new_tasks_listbox.size() and not in_progress_listbox.size() and not closed_tasks_listbox.size():
        messagebox.showwarning("Move to Work", "No tasks to move to work.")
    else:
        messagebox.showwarning("Move to Work", "No selected tasks to move to work.")
...
# Add event handler for Ctrl+Click
def on_click(event):
    if event.state & 0x0004:  # Ctrl+Click
        mark_task()

# Add event handler for mouse selection
def on_select(event):
    if event.state & 0x0004:  # Ctrl+Click
        mark_task()
...
task_label = tk.Label(root, text="Enter Task:", bg=bg_color, fg=label_color)
...
add_button = tk.Button(buttons_frame, text="Add to New Tasks", bg=button_color, fg=task_color, command=add_to_new)
add_button.grid(row=0, column=0, padx=5)

delete_button = tk.Button(buttons_frame, text="Delete Task", bg=button_color, fg=task_color, command=delete_task)
delete_button.grid(row=0, column=1, padx=5)

mark_button = tk.Button(buttons_frame, text="Select Task", bg=button_color, fg=task_color, command=mark_task)
mark_button.grid(row=0, column=2, padx=5)

work_button = tk.Button(buttons_frame, text="Move to Work", bg=button_color, fg=task_color, command=move_to_work)
work_button.grid(row=0, column=3, padx=5)

close_button = tk.Button(buttons_frame, text="Close Task", bg=button_color, fg=task_color, command=close_task)
...
# New Tasks Column
new_tasks_label = tk.Label(tasks_frame, text="New Tasks", bg=bg_color, fg=label_color)
...
# In Progress Column
in_progress_label = tk.Label(tasks_frame, text="Tasks in Progress", bg=bg_color, fg=label_color)
...
# Closed Tasks Column
closed_tasks_label = tk.Label(tasks_frame, text="Closed Tasks", bg=bg_color, fg=label_color)
...
# Bind event handlers to task lists
for listbox in [new_tasks_listbox, in_progress_listbox, closed_tasks_listbox]:
